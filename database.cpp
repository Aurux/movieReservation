...
bool Database::createTables(){
    ...
    if (query.exec("CREATE TABLE IF NOT EXISTS showtimes (id int NOT NULL AUTO_INCREMENT, screenID int, title VARCHAR(255), showtime TIME, FOREIGN KEY(screenID) REFERENCES screens(screenID), PRIMARY KEY (id));")){
        ...
        qDebug() << "Table showtimes failed to create: " << query.lastError().text();
        success = false;
...
bool Database::addExamples(){
   ...
   if(!query.exec("INSERT INTO showtimes (screenID, title, showtime) VALUES (1, 'The Batman', '21:00:00'), (1, 'The Batman', '17:00:00'), (3, 'Hunt for the Wilderpeople', '11:30:00'), (2, 'American Psycho', '21:30:00'), (1, 'Interstellar', '12:30:00'), (2, 'Moonfall', '13:30:00');")){
       qDebug() << "Failed to insert showtimes: " << query.lastError().text();
       success = false;
   }

   if(!query.exec("INSERT INTO staff (staffid) VALUES (10000), (10001), (10002)")){
       qDebug() << "Failed to insert staff: " << query.lastError().text();
       success = false;
   }
   return success;
